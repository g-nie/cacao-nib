...
def _process_checked(
    ...
    emits/resolves. Prints nothing except a read-error notice, for which it
    returns `None` (a read error has nothing worth caching and is re-attempted
    next run)."""
...
def _emit(files, cache_hits, results, session: cache.Session) -> int:
    """Walk `files` in order, emitting as we go: replay a cache hit inline, else
    pull the next checked result (misses arrive in `files` order, so they line
    up). Immediate findings stream as we go; every file also contributes its
    import targets and any deferred findings. Once the whole tree is seen we
    resolve the deferred findings against the project's imported-module set and
    print the survivors at the end, in file order. Returns the total issue count.

    Each file's `(diags, targets, deferred)` is recorded for next run. Deferred
    findings are re-resolved every run from those cached targets, so a cached
    verdict never goes stale when some *other* file's imports change.

    `closing` matters for the parallel branch: we pull exactly one result per
    miss and never exhaust the generator, so closing it is what runs its cleanup
    (releasing the worker subinterpreters)."""
    issues = 0
    ...
    if deferred_holds:
        ...
        imported = imported_among(gated, targets_per_file)
        for file_str, deferred in deferred_holds:
            survivors = [
                d[:4] for d in deferred if _keep_deferred(d[4], d[5], imported)
            ]
...
def _run_cli() -> int:
    ...
    # default cache lives under the project root. Cross-file (deferred) findings
    # are re-resolved every run from the cached import targets, so the cache stays
    # correct even when a finding hinges on some *other* file's imports.
    ...
    issues = _emit(files, cache_hits, results, session)
    session.flush()
