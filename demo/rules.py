...
class NoUnimportedSetup(Rule):
    ...
    by name. A plugin/extension hook only does anything if something imports and
    calls it, so a `setup` nobody pulls in is dead wiring.
    ...
    and the `UnimportedDiagnostic` defers the "is it imported anywhere"
    verdict — checked against every file's `from … import …` targets — to the end
    of the run."""
