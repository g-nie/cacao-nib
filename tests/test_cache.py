"""Tests for the on-disk result cache (`nib.cache`): the `is_changed` decision
table, atomic load/write round-trips, ruleset-hash keying, and an end-to-end
check that an unchanged clean file is skipped on a re-run."""
...
# --- is_changed decision table (mirrors Black) -----------------------------
...
def test_write_then_load_round_trip(tmp_path):
    ...
    c = {"/abs/a.py": cache.FileData(1.5, 10, "deadbeef")}
    cache.write(path, c)
    assert path.is_file()
